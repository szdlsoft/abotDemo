    ...
    class Program
    {
        ...
        private static IWebCrawler GetCustomBehaviorUsingLambdaWebCrawler()
        {
            ...
            crawler.ShouldDownloadPageContent((crawledPage, crawlContext) =>
            {
                ...
                {
                    decision = new CrawlDecision { Allow = false, ShouldStopCrawl=true, Reason = "不是主页或价格信息" };
                }
            ...
            crawler.ShouldCrawlPageLinks((crawledPage, crawlContext) =>
            {
                ...
                return new CrawlDecision { Allow = false, Reason = "需要手工爬链接" };
            });

            return crawler;
        }

        private static void Log(string method, string absoluteUri)
        {
            log4net.LogManager.GetLogger("AbotLogger").Info($"{method} {absoluteUri}");
        }
        ...
        {
            Log("crawler_ProcessPageCrawlStarting", e.PageToCrawl.Uri.AbsoluteUri);
        ...
        {
            Log("crawler_ProcessPageCrawlCompleted", e.CrawledPage.Uri.AbsoluteUri);

            var crawledPage = e.CrawledPage;
            var crawlContext = e.CrawlContext;
            if (crawledPage.Uri == crawlContext.RootUri) //确定价格信息uri
            {
                Uri priceUri = GetPriceUri(crawledPage.AngleSharpHtmlDocument);
                crawlContext.CrawlBag.PriceUri = priceUri;
                crawlContext.Scheduler.Add( new PageToCrawl(priceUri)
                {
                });
                return;
            }
            ...
            //处理价格信息
            if (crawledPage.Uri == crawlContext.CrawlBag.PriceUri )
            {
                ShowPrice(crawledPage.AngleSharpHtmlDocument);
            }
        }
        ...
        private static Uri GetPriceUri(IHtmlDocument htmlDocument)
        {
        ...
        {
            Log("crawler_PageCrawlDisallowed", e.PageToCrawl.Uri.AbsoluteUri);
            var e1 = e;
            //Process data
        }
    }
